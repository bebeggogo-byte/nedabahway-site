...
> 새 세션은 이 문서를 읽고 곧바로 이어서 진행한다. 갱신일: 2026-06-11 (JOS 완주 시점)
...
- 진행: **714 / 1,189장 (60%)** — 오경 187 + 여호수아 24 + 시가서 243 + 신약 260
- 완주 권 34권: 오경 5 + JOS + 시가서 5 + 신약 27 (각 권·장 PDF 아티팩트 배포 완료)
- 흐름(flow) 도출 완료 권: 신약 27 + 시가서 5 + **역사서 12권(JOS~EST 전부, 2026-06-11 도출)** / 미도출: 선지서 17권(ISA~MAL) — 착수 전 §10.2 흐름 도출 5문 필수

## 지금 하던 일 — 역사서 진행 (다음: 사사기 JDG 21장)
- 순서: ~~JOS 24~~✅ → **JDG 21** → RUT 4 → 1SA 31 → 2SA 24 → 1KI 22 → 2KI 25 → 1CH 29 → 2CH 36 → EZR 10 → NEH 13 → EST 10 (역사서 잔여 225장) → 선지서 250장
- 역사서 flow는 전부 book-telos.json에 기록됨 — 바로 장 생산 가능
- 골든 예시: `magazine/JOS/1/index.html` (역사서 내러티브 규격의 표준) — genre '내러티브(세부)'
- 메인 세션이 장별 사양(절수·핵심 구절·관찰 포인트·원어 후보·next-row)을 프롬프트에 명시해 6장 병렬 배치로 위임하는 패턴이 검증됨 (JOB~JOS 전 구간 1회 통과율 ~100%)
