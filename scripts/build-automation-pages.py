...
.guide-body h2 {{ font-size:1.35rem; margin-top:2.5rem; padding-left:.6rem; border-left:4px solid #1d4ed8; color:#1b1b1b; }}
.guide-body h3 {{ font-size:1.1rem; margin-top:1.6rem; color:#1b1b1b; }}
.guide-body h4 {{ font-size:1rem; margin-top:1.2rem; color:#6a604f; }}
.guide-body blockquote {{ background:#f1ede5; border-left:4px solid #1d4ed8; padding:.6rem 1.2rem; color:#1b1b1b; margin:1rem 0; border-radius:0 8px 8px 0; }}
.guide-body table {{ width:100%; border-collapse:collapse; margin:1rem 0; font-size:.95rem; }}
.guide-body th {{ background:#f1ede5; text-align:left; padding:.55rem .7rem; border-bottom:2px solid #e5d8c4; }}
...
.guide-body a {{ color:#1d4ed8; }}
.tag-pill {{ display:inline-block; font-size:.72rem; letter-spacing:.14em; font-weight:700; padding:.22rem .6rem; border-radius:99px; background:#f1ede5; color:#1d4ed8; border:1px solid #e5d8c4; }}
.guide-meta {{ color:#6a604f; font-size:.92rem; margin-top:.4rem; }}
.starter-card {{ display:grid; grid-template-columns:repeat(auto-fit, minmax(140px, 1fr)); gap:.8rem; max-width:780px; margin:1.4rem auto 0; padding:1rem 1.2rem; background:#f1ede5; border:1px solid #e5d8c4; border-radius:10px; font-family:'Noto Sans KR',sans-serif; }}
.starter-card .si {{ font-size:.72rem; color:#665f57; letter-spacing:.1em; font-weight:700; text-transform:uppercase; }}
.starter-card .sv {{ font-size:1rem; color:#1b1b1b; margin-top:.2rem; font-weight:600; }}
.related-aside {{ margin:3rem auto 4rem; max-width:780px; padding:1.6rem 1.8rem; background:#f1ede5; border-radius:12px; }}
...
<nav class="gnav" role="navigation" aria-label="주요 메뉴">
  <div class="gnav__inner">
    <a href="/" class="gnav__logo" aria-label="네다바웨이 홈"><img class="gnav__logo-sym" src="/assets/brand/nw-symbol.png" width="314" height="202" alt=""><img class="gnav__logo-word" src="/assets/brand/nw-wordmark.png" width="421" height="72" alt="NEDABAHWAY"></a>
    <ul class="gnav__links">
      <li><a href="/auto/tools/" class="gnav__link">도구 모음</a></li>
      <li><a href="/resources/automation/" class="gnav__link">자료 허브</a></li>
...
<header style="max-width:780px;margin:2.4rem auto 0;padding:0 1.4rem;font-family:'Noto Sans KR',sans-serif;">
  <p style="font-size:.78rem;color:#1d4ed8;letter-spacing:.18em;font-weight:700;">RESOURCE · {section_upper} 자동화 · <span class="tag-pill">{tag}</span></p>
</header>
...
<section style="max-width:780px;margin:1.2rem auto 0;padding:1rem 1.3rem;background:linear-gradient(135deg,#fff8ea 0%, #fbe9c5 100%);border:1px solid #e8d2a4;border-radius:10px;font-family:'Noto Sans KR',sans-serif;display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
  <div style="flex:1;min-width:240px;">
    <strong style="display:block;color:#1b1b1b;font-size:.98rem;">⚡ 셋업 전에 결과부터 만들어보기</strong>
    <span style="color:#6a604f;font-size:.86rem;">설치·코딩 없이 입력만으로 결과를 가져갈 수 있는 도구가 따로 있습니다.</span>
  </div>
  <a href="{tool_url}" style="display:inline-block;background:#1d4ed8;color:#fff;padding:.65rem 1.2rem;border-radius:6px;text-decoration:none;font-weight:700;font-size:.92rem;">🚀 {tool_label} →</a>
</section>
...
def main() -> None:
    for slug, meta in CARDS.items():
        ...
        page = PAGE_TPL.format(
            ...
            og_image=OG_IMAGE,
            tool_url=meta.get("tool", "/auto/tools/"),
            tool_label=meta.get("tool_label", "도구 모음 열기"),
            body=body_html,
        )
        page = page.replace(
            '<article class="guide-body">\n',
