...
def render_chapter(code: str, kr: str, en: str, ch: int, book_sbm_dir: str, total: int, language: str, block: str) -> str:
    ...
    if has_data:
        body_sections += f"""
        <section class="stage stage--raw" id="s1">
          <span class="stage__num">단계 1</span>
          <h2 class="stage__title">본문 그대로 읽기.</h2>
          <p>본문은 본문이 자기 시간 안에서 스스로 풀리도록 둡니다. 옮긴 말을 옮긴 그대로 받아 적습니다.</p>
          <div class="md-body">{md_to_html(raw[:8000])}</div>
        </section>

        <section class="stage" id="s2">
          <span class="stage__num">단계 2~6</span>
          <h2 class="stage__title">관찰된 사실 — 단어 빈도 · 좌표 · 동사 · 대조 · 장 경계.</h2>
          <div class="md-body">{md_to_html(facts[:12000])}</div>
        </section>

        <section class="stage" id="s8">
          <span class="stage__num">단계 7~9</span>
          <h2 class="stage__title">미해결 질문과 관찰자의 자리.</h2>
          <p>본문이 답하지 않는 자리는 답하지 않은 채 둡니다.</p>
